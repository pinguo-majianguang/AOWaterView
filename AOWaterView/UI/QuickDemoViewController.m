...
- (QuickDemoViewController *)initWithRoot:(QRootElement *)rootElement
{
    ...
    if (self)
    {
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(responseDidReceivedNotify:) name:kResponseDidReceived object:nil];
    }
    return self;
}
...
- (void)viewDidLoad
{
    ...
    self.tencentOAuth = [sdkCall getinstance].oauth;
    [super viewDidLoad];
}
